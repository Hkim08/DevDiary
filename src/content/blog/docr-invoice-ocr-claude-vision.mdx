...
This is the main design constraint: Claude's API is called directly from the browser. The user's API key is exposed in the browser's dev tools. For a local development tool that runs on the user's machine, this is acceptable. For a production SaaS, you would add a lightweight proxy server between the browser and Anthropic.
